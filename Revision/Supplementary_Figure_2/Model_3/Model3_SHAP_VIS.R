...
## 16b. SHAP robustness across random seeds and cohorts --------

shap_seeds <- c(123, 456, 789, 1011, 2025)
N_TOP <- 20

get_shap_importance <- function(seed_value, X_data, cohort_name) {
  set.seed(seed_value)
  
  shap_values <- fastshap::explain(
    xgb_engine,
    X = X_data,
    pred_wrapper = p_fun_processed,
    nsim = 25
  )
...
# Complete cases for test data
complete_test_idx <- which(complete.cases(X_test_processed))
X_test_processed_complete <- X_test_processed[complete_test_idx, ]

# Run SHAP across seeds for train and test cohorts
shap_robustness <- bind_rows(
  lapply(shap_seeds, get_shap_importance,
         X_data = X_train_processed_complete,
         cohort_name = "Training"),
  lapply(shap_seeds, get_shap_importance,
         X_data = X_test_processed_complete,
         cohort_name = "Test")
)

# Top 20 features per seed/cohort
top20_by_seed <- shap_robustness %>%
  group_by(Seed, Cohort) %>%
  slice_min(order_by = Rank, n = N_TOP) %>%
  ungroup()

# Frequency of appearance in top 20
top20_frequency <- top20_by_seed %>%
  ...
  arrange(Cohort, desc(Times_in_top20), Mean_rank)

print(top20_frequency)

# Train-test overlap for each seed
train_test_overlap <- top20_by_seed %>%
  select(Seed, Cohort, Feature) %>%
  group_by(Seed) %>%
  summarise(
    Train_Test_Overlap = length(intersect(
      Feature[Cohort == "Training"],
      Feature[Cohort == "Test"]
    )),
    Percent_Overlap = Train_Test_Overlap / N_TOP * 100,
    .groups = "drop"
  )

print(train_test_overlap)

# Overall robust features appearing repeatedly across seeds/cohorts
robust_features <- top20_by_seed %>%
  group_by(Feature) %>%
  summarise(
    Total_appearances = n(),
    Appears_in_training = any(Cohort == "Training"),
    Appears_in_test = any(Cohort == "Test"),
    Mean_rank = mean(Rank),
    .groups = "drop"
  ) %>%
...
write_csv(shap_robustness, "Model3_SHAP_all_seed_rankings.csv")
write_csv(top20_frequency, "Model3_SHAP_top20_frequency_by_cohort.csv")
write_csv(train_test_overlap, "Model3_SHAP_train_test_overlap.csv")
write_csv(robust_features, "Model3_SHAP_robust_features.csv")
