...
train_preds <- predict(fit_final_m3, new_data = train_m3, type = "prob") %>%
  bind_cols(
    predict(fit_final_m3, new_data = train_m3, type = "class"),
    train_m3 %>% dplyr::select(resp_bin, dbgap_rnaseq_sample)
  )
