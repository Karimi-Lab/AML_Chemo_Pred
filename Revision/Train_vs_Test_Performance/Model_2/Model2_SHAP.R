...
## 11a. Evaluate on training set ------------------------------

train_preds <- predict(fit_final_m1, new_data = train_m1, type = "prob") %>%
  bind_cols(
    predict(fit_final_m1, new_data = train_m1, type = "class"),
    train_m1 %>% dplyr::select(resp_bin, dbgap_rnaseq_sample)
  )

cat("\n=== TRAIN PERFORMANCE (Model", MODEL_ID, ") ===\n")

cat("\n--- Train Probability-Based Metrics ---\n")
train_prob_metrics <- metric_set(roc_auc, pr_auc)(
  data = train_preds,
  truth = resp_bin,
  .pred_Refractory,
  event_level = "second"
)
print(train_prob_metrics)

write_csv(
  train_prob_metrics,
  "model_2_train_metrics.csv"
)
